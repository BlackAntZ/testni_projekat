...
const Sellers = () => {
  ...
  return (
    <>
      <h1>Sellers</h1>
      <CrudActions idList={selectedRows} invoices={data.invoices} type={'seller'}></CrudActions>
      <Table columns={columns} pagination={paginationData} setPagination={setPaginationData} items={sellers.length}>
        {sellers.map(row => {
            ...
            return <SellerRow key={row.id} row={row} selectedRows={selectedRows} selectRowHandler={selectRowHandler} columns={columns} />
          }
        )}
      </Table>
      <Outlet />
...
export async function loader () {
  const response = await fetch('https://63ce9ae9fdfe2764c726a809.mockapi.io/sellers');
  ...
  if (!response.ok) {
    ...
    return {sellers: await response.json(), invoices: await invoiceResponse.json()};
  }
}