...
const Invoices = () => {
  ...
  return (
    <>
      ...
      <Table columns={columns} pagination={paginationData} setPagination={setPaginationData} items={data.length}>
        {data.map(row => {
          ...
          return <InvoiceRow key={row.id} row={row} selectedRows={selectedRows} selectRowHandler={selectRowHandler} columns={columns} />
        }
        )}
      </Table>
      <Outlet />
